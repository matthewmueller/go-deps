...
// FindTests mains in a variety of ways
// returning an array of absolute
// paths.
//
// Mostly based on:
// https://github.com/mitchellh/gox/blob/c9740af9c6574448fd48eb30a71f964014c7a837/go.go#L123
func FindTests(packages ...string) (mains []string, err error) {
	cwd, err := os.Getwd()
	if err != nil {
		return mains, err
	}
	...
	args := make([]string, 0, len(packages)+3)
	args = append(args, "list", "-f", `{{ join .TestGoFiles "," }}|{{ join .XTestGoFiles "," }}|{{ .ImportPath }}`)
	args = append(args, packages...)
	output, err := execGo(goCmd, nil, "", args...)
	if err != nil {
		return nil, err
	...
	for _, line := range strings.Split(output, "\n") {
		...
		var tests []string
		for _, test := range strings.Split(parts[0], ",") {
			if test == "" {
				continue
			}
		...
		for _, test := range strings.Split(parts[1], ",") {
			if test == "" {
				continue
			}
			tests = append(tests, test)
		...
		for _, test := range tests {
			if parts[2] == "command-line-arguments" {
				// TODO: i don't think this will work for multiple files
				for _, pkg := range packages {
					results = append(results, path.Join(cwd, filepath.Dir(pkg)))
				}
			...
			if strings.HasPrefix(test, "_/") {
				results = append(results, strings.TrimPrefix(test, "_"))
				continue
			}
